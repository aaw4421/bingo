  ...
  version ? `&version=${version}` : ""
    ...
    <ul>
      <li>
        <i>${localize(
          "Items"
        )}</i>${localize(": item must be present in the inventory (armor slots also count) in order to finish. For example, for the goal <i>Ancient Debris</i>, you must not smelt it")}
      </li>
      <li>
        <i>${localize(
          "Kill a(n) X"
    ...
    <ul>
      <li>${localize("Glitches that duplicate items are banned")}</li>
      <li>${localize("X-ray glitches are allowed")}</li>
      <li>${localize("Wrong warping using Nether/End portals is allowed")}</li>
